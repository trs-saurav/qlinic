...
export default async function middleware(req) {
  ...
  if (isAuthPath) {
    ...
    if (isLoggedIn && nextUrl.pathname === '/sign-in') {
      ...
      // If user is logged in and on wrong subdomain, redirect to correct subdomain
      const userSubdomainMap = {
        'admin': 'admin',
      ...
      const expectedSubdomain = userSubdomainMap[userRole];
      if (expectedSubdomain && expectedSubdomain !== roleFromSubdomain) {
        const port = isDevelopment ? ':3000' : '';
        const protocol = isDevelopment ? 'http' : 'https';
        
        const correctSubdomainUrl = new URL(
          `${protocol}://${expectedSubdomain}.${mainDomain}${port}${nextUrl.pathname}${nextUrl.search}`
        );
    ...
    // Enhanced check: If not logged in and on wrong subdomain for intended role, block access
    if (!isLoggedIn && nextUrl.pathname === '/sign-in' && roleFromSubdomain) {
      // If user is trying to access sign-in on a subdomain, they should only be able to sign in with matching role
      // We'll let the backend auth handle the validation, but we can still validate here
      
      // This prevents users from accessing sign-in pages on wrong subdomains
      if (requestedRole && requestedRole !== roleFromSubdomain) {
        // If they're requesting a specific role that doesn't match the subdomain, redirect
        const signInUrl = new URL('/sign-in', req.url);
        signInUrl.searchParams.set('role', roleFromSubdomain);
        signInUrl.searchParams.set('error', 'invalid_subdomain_role');
  ...
  if (isLoggedIn && roleFromSubdomain && roleFromSubdomain !== 'api') {
    // Check if user is on the correct subdomain for their role
    // Enforce that users can only access their designated subdomain
    const userSubdomainMap = {
      'admin': 'admin',
      'hospital_admin': 'hospital',
      'doctor': 'doctor',
    ...
    const expectedSubdomain = userSubdomainMap[userRole];
    const actualSubdomain = roleFromSubdomain;
    
    // If logged-in user is on wrong subdomain, redirect them to the correct one
    if (expectedSubdomain && expectedSubdomain !== actualSubdomain) {
      const port = isDevelopment ? ':3000' : '';
      const protocol = isDevelopment ? 'http' : 'https';
    ...
    // Only restrict access based on user's role
