...
const roles = [
  {
    ...
    features: [
      ...
      { icon: FileText, text: 'Digital records' },
      { icon: UserPlus, text: 'Family profiles' },
    ]
  },
  {
    value: 'doctor',
    ...
    icon: Stethoscope,
    features: [
      { icon: ClipboardList, text: 'Smart scheduling' },
      { icon: FileText, text: 'Patient history' },
      { icon: Hospital, text: 'Multi-hospital' },
    ]
  },
  {
    value: 'hospital_admin',
    label: 'Hospital', // Shortened for mobile
    description: 'Manage operations & staff',
    icon: Building2,
    features: [
      { icon: Users, text: 'Staff management' },
      { icon: Calendar, text: 'Oversight' },
...
export default function SignUpPage() {
  ...
  return (
    <div className="h-screen w-screen overflow-hidden relative bg-[#F8FAFC] dark:bg-[#020617] flex items-center justify-center">
      ...
      {/* Background Blobs - Kept from previous design */}
      ...
      <div className="relative z-10 w-full max-w-5xl px-4 flex flex-col items-center h-full justify-center">
        ...
        >
          <Link href="/" className="inline-flex items-center gap-2 group">
            ...
            <span className="text-xl md:text-2xl font-bold text-slate-800 dark:text-slate-100 tracking-tight">
              Qlinic
            </span>
          </Link>
        </motion.div>
        ...
        <AnimatePresence mode="wait">
          {!selectedRole ? (
            ...
            >
              ...
              {/* Grid: 1 col mobile (compact), 3 cols desktop (detailed) */}
              <div className="grid grid-cols-1 md:grid-cols-3 gap-3 md:gap-5">
                {roles.map((role, index) => {
                  ...
                  return (
                    ...
                    >
                      <Card className="h-full border-0 bg-white/40 dark:bg-slate-900/40 backdrop-blur-[40px] shadow-sm hover:shadow-md transition-all rounded-2xl md:rounded-[2rem] ring-1 ring-white/50 dark:ring-white/10">
                        <CardContent className="p-4 md:p-6 flex items-center md:flex-col md:items-start h-full gap-4 md:gap-0">
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            {/* Short description on desktop, hidden/short on mobile */}
                            <p className="text-xs md:text-sm text-slate-500 dark:text-slate-400 md:mb-4 truncate md:whitespace-normal">
                              {role.description}
                            </p>
                            ...
                            <div className="hidden md:block space-y-2 mt-auto">
                              {role.features.map((f, i) => (
                                <div key={i} className="flex items-center gap-2">
                                  ...
                                  <span className="text-xs text-slate-600 dark:text-slate-300">{f.text}</span>
                                </div>
                              ))}
                            </div>
                          </div>
              ...
              <div className="text-center mt-6">
                <Link href="/sign-in" className="text-xs md:text-sm text-slate-500 hover:text-blue-600 font-medium">
                  Already have an account? <span className="underline">Sign in</span>
                </Link>
              </div>
            </motion.div>
          ) : (
            // SIGN UP FORM - Compact Mobile
            <motion.div
              key="signup-form"
              initial={{ opacity: 0, scale: 0.95 }}
              animate={{ opacity: 1, scale: 1 }}
            ...
            >
              ...
              <button
                ...
                className="flex items-center text-xs md:text-sm font-medium text-slate-500 hover:text-slate-800 mb-3 md:mb-5 pl-1"
              >
                <ArrowLeft className="w-3 h-3 md:w-4 md:h-4 mr-1.5" />
                Back to roles
              </button>

              <Card className="border-0 bg-white/40 dark:bg-slate-900/40 backdrop-blur-[40px] shadow-sm rounded-[2rem] ring-1 ring-white/50 dark:ring-white/10">
                <CardContent className="p-5 md:p-8">
                  
                  {/* Form Header */}
                  <div className="text-center mb-5 md:mb-6">
                    <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-blue-50 dark:bg-blue-900/20 mb-2">
                      <selectedRoleObj.icon className="w-3 h-3 md:w-4 md:h-4 text-blue-600" />
                      <span className="text-xs font-bold text-blue-700 dark:text-blue-300">{selectedRoleObj.label}</span>
                    </div>
                  ...
                  <div className="grid grid-cols-3 gap-2 mb-5 md:mb-6">
                    {['google', 'facebook', 'apple'].map((provider) => (
                      <button
                        ...
                        className="flex items-center justify-center h-9 md:h-10 rounded-xl bg-white/60 dark:bg-slate-800/60 ring-1 ring-slate-200 dark:ring-slate-700 hover:bg-white/80 transition-all"
                      >
                    ...
                    <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-3">
                      <div className="grid grid-cols-2 gap-3">
                        <FormField control={form.control} name="firstName" render={({ field }) => (
                          <FormItem><FormControl><Input placeholder="First Name" className="h-10 md:h-11 rounded-xl bg-white/50 border-0 ring-1 ring-slate-200 px-3 text-sm" {...field} /></FormControl><FormMessage className="text-[10px]" /></FormItem>
                        )} />
                        <FormField control={form.control} name="lastName" render={({ field }) => (
                          <FormItem><FormControl><Input placeholder="Last Name" className="h-10 md:h-11 rounded-xl bg-white/50 border-0 ring-1 ring-slate-200 px-3 text-sm" {...field} /></FormControl><FormMessage className="text-[10px]" /></FormItem>
                        )} />
                      </div>

                      <FormField control={form.control} name="email" render={({ field }) => (
                        <FormItem><FormControl><Input type="email" placeholder="Email Address" className="h-10 md:h-11 rounded-xl bg-white/50 border-0 ring-1 ring-slate-200 px-3 text-sm" {...field} /></FormControl><FormMessage className="text-[10px]" /></FormItem>
                      )} />
                        ...
                        <FormItem>
                          <FormControl>
                            <div className="relative">
                              <Input type={showPassword ? 'text' : 'password'} placeholder="Password" className="h-10 md:h-11 rounded-xl bg-white/50 border-0 ring-1 ring-slate-200 px-3 pr-9 text-sm" {...field} />
                      ...
                      <FormField control={form.control} name="confirmPassword" render={({ field }) => (
                        <FormItem>
                          <FormControl>
                            <div className="relative">
                              <Input type={showConfirmPassword ? 'text' : 'password'} placeholder="Confirm Password" className="h-10 md:h-11 rounded-xl bg-white/50 border-0 ring-1 ring-slate-200 px-3 pr-9 text-sm" {...field} />
              ...
              <div className="text-center mt-4">
                <p className="text-[10px] text-slate-500">
                  By joining, you agree to our <span className="underline cursor-pointer">Terms</span> & <span className="underline cursor-pointer">Privacy</span>
                </p>
              </div>
            </motion.div>
          )}
