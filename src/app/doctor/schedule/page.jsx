...
function HospitalScheduleRow({ affiliation, onManageSchedule }) {
  ...
  return (
    <div 
      className="group flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-4 rounded-xl border border-slate-200 bg-white hover:border-blue-200 hover:shadow-md transition-all duration-200 cursor-pointer" 
      onClick={onManageSchedule}
    >
      <div className="flex items-center gap-4 min-w-0">
        <div className="relative h-14 w-14 sm:h-16 sm:w-16 shrink-0 rounded-lg overflow-hidden border border-slate-100 bg-slate-50">
           {hospital?.images?.[0] ? (
              <Image 
        ...
        <div className="flex-1 min-w-0 space-y-1">
           <div className="flex items-center gap-2">
              <h3 className="font-bold text-slate-900 truncate text-base sm:text-lg group-hover:text-blue-700 transition-colors">
                 {hospital?.name || 'Unknown Hospital'}
              </h3>
              <Badge variant={hasSchedule ? "default" : "secondary"} className={`h-5 text-[10px] px-1.5 ${hasSchedule ? 'bg-green-100 text-green-700 hover:bg-green-100' : 'bg-slate-100 text-slate-500'}`}>
      ...
      <div className="flex items-center justify-between sm:justify-end gap-3 pl-16 sm:pl-0">
         {hasSchedule ? (
            <div className="hidden sm:flex flex-col items-end text-xs text-slate-500 mr-2">
               <span className="font-medium text-slate-700">Weekly Schedule</span>
               <span>{affiliation.weeklySchedule?.length || 0} days configured</span>
            </div>
         ) : (
            <span className="hidden sm:block text-xs text-orange-600 font-medium bg-orange-50 px-2 py-1 rounded-md">
               No slots added
            </span>
         )}
      ...
      <div className="p-8 max-w-2xl mx-auto mt-10">
        <Card className="text-center py-12 border-dashed shadow-sm">
          <CardContent>
            ...
            <h2 className="text-xl font-bold mb-2 text-slate-900">No Active Clinics Found</h2>
    ...
    <div className="container mx-auto p-4 max-w-5xl space-y-8">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 border-b border-slate-200 pb-5">
        <div>
          ...
          <p className="text-slate-500 mt-1">Configure weekly hours and slot durations for your clinics.</p>
        </div>
      ...
      <Dialog open={scheduleDialogOpen} onOpenChange={setScheduleDialogOpen}>
        <DialogContent className="w-[95vw] max-w-3xl h-[85vh] flex flex-col p-0 gap-0 overflow-hidden sm:rounded-2xl">
          <DialogHeader className="p-5 border-b bg-white shrink-0">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                ...
                <div className="flex items-center gap-3 bg-slate-50 px-3 py-1.5 rounded-lg border border-slate-100 self-start sm:self-auto">
                    ...
                    >
                      <SelectTrigger className="w-[90px] h-7 bg-white text-xs font-semibold border-slate-200 shadow-sm focus:ring-0">
                        <SelectValue />
                      </SelectTrigger>
                      <SelectContent>
            ...
            <div className="flex-1 overflow-hidden flex flex-col bg-slate-50/50">
              <Tabs defaultValue="weekly" className="flex-1 flex flex-col overflow-hidden">
                <div className="px-6 pt-4 shrink-0">
                  <TabsList className="bg-white border border-slate-200 p-1 h-auto rounded-lg inline-flex shadow-sm">
                ...
                <TabsContent value="weekly" className="flex-1 overflow-y-auto p-4 sm:p-6 m-0">
                  <div className="bg-white rounded-xl border border-slate-200 shadow-sm divide-y divide-slate-100">
                    {DAYS.map(day => {
                      ...
                      return (
                        <div key={day} className="p-4 flex flex-col sm:flex-row gap-4 sm:items-start group transition-colors hover:bg-slate-50/50">
                           {/* Day Column */}
                              ...
                              {hasSlots ? (
                                 <div className="flex flex-wrap gap-2">
                                    {dayData.slots.map((slot, idx) => (
                                       <div key={idx} className="flex items-center gap-2 bg-slate-50 border border-slate-200 rounded-md pl-3 pr-1 py-1.5 group/slot hover:border-red-200 hover:bg-red-50 transition-colors">
                                          ...
                                          {slot.room && (
                                             <span className="text-[10px] bg-white border px-1 rounded text-slate-500">
                                                {slot.room}
                                             </span>
                                          )}
                                          <button 
                                             onClick={() => handleRemoveSlot(day, idx)}
                                             className="ml-1 p-1 text-slate-400 hover:text-red-600 rounded-full hover:bg-white transition-all opacity-0 group-hover/slot:opacity-100"
                                          >
                                 ...
                                 <div className="flex items-center gap-2 py-1">
                                    ...
                                    <span className="text-xs text-slate-400 italic">No availability set</span>
                                 </div>
                              )}
                           </div>
                           ...
                           <div className="shrink-0 pt-0 sm:pt-0">
                              <Button 
                                 ...
                                 className={`h-8 text-xs ${hasSlots ? 'text-blue-600 border-blue-200 hover:bg-blue-50' : 'text-slate-500'}`}
                                 onClick={() => {
                                    setActiveDay(day)
                ...
                <TabsContent value="exceptions" className="flex-1 overflow-y-auto p-4 sm:p-6 m-0">
                    <div className="max-w-3xl mx-auto space-y-6">
                      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white p-5 rounded-xl border border-slate-200 shadow-sm">
                        ...
                        <Button onClick={() => setExceptionDialogOpen(true)} size="sm" className="bg-slate-900 text-white hover:bg-slate-800 w-full sm:w-auto">
                      ...
                      {exceptions.length === 0 ? (
                        <div className="text-center py-16 border-2 border-dashed rounded-xl bg-slate-50/50">
                          ...
                          <h3 className="text-sm font-bold text-slate-900">No Exceptions Configured</h3>
                          <p className="text-xs text-slate-500 mt-1">Your schedule follows the weekly plan strictly.</p>
                        </div>
                      ) : (
                        <div className="grid gap-3">
                          {exceptions.map((ex, idx) => (
                            <div key={idx} className="flex items-center justify-between p-4 bg-white rounded-lg border border-slate-200 shadow-sm hover:border-blue-200 transition-all">
          ...
          <div className="p-4 sm:p-5 border-t bg-white shrink-0 flex flex-col-reverse sm:flex-row justify-end gap-3 z-10">
