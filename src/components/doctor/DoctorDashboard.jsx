...
export default function DoctorDashboard() {
  ...
  // ✅ Build normalized hospital list from BOTH affiliations AND appointments
  const availableHospitals = useMemo(() => {
    const hospitalsMap = new Map()
    
    // Add from affiliations
    doctor?.affiliations?.forEach(aff => {
      if (aff.status === 'APPROVED') {
        const hId = aff.hospitalId?._id || aff.hospitalId
        const hName = aff.hospitalId?.name || 'Unknown Clinic'
        if (hId && !hospitalsMap.has(hId)) {
          hospitalsMap.set(hId, { id: hId, name: hName, source: 'affiliation' })
        }
      }
    })
    
    // Add from appointments (in case affiliation data is missing)
    appointments?.forEach(apt => {
      const hId = apt.hospitalId?._id || apt.hospitalId
      const hName = apt.hospitalId?.name
      if (hId && hName && !hospitalsMap.has(hId)) {
        hospitalsMap.set(hId, { id: hId, name: hName, source: 'appointment' })
      }
    })
    
    return Array.from(hospitalsMap.values())
  }, [doctor?.affiliations, appointments])

  // ✅ Smart Hospital Selection
  useEffect(() => {
    if (!selectedHospitalId && availableHospitals.length > 0) {
      // Try to find hospital with today's appointments first
      const todayHospital = appointments?.find(apt => 
        new Date(apt.scheduledTime).toDateString() === new Date().toDateString()
      )
      
      if (todayHospital) {
        const hId = todayHospital.hospitalId?._id || todayHospital.hospitalId
  ...
  }, [availableHospitals, appointments, selectedHospitalId])
  ...
  // ✅ Filter appointments for selected hospital
  ...
  // ✅ Get current hospital name from normalized list
  const currentHospitalName = availableHospitals.find(h => h.id === selectedHospitalId)?.name || 'Select Clinic'

  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 pb-20 font-sans">
      
      {/* HEADER */}
      <div className="bg-gradient-to-br from-blue-600 to-blue-700 pt-6 pb-20 px-4 rounded-b-[2.5rem] shadow-xl relative">
        ...
        <div className="flex items-center justify-between mb-8 relative z-10">
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button variant="ghost" className="bg-white/10 hover:bg-white/20 text-white border border-white/20 rounded-xl pl-3 pr-4 h-12 max-w-[240px] backdrop-blur-sm">
                <div className="flex flex-col items-start mr-2 overflow-hidden text-left flex-1">
            ...
            <DropdownMenuContent align="start" className="w-64">
              ...
              {availableHospitals.length > 0 ? (
                ...
                <DropdownMenuItem disabled>
                  <span className="text-sm text-slate-400">No clinics found</span>
                </DropdownMenuItem>
              )}
            </DropdownMenuContent>
          </DropdownMenu>
          ...
          {/* Profile Button */}
          <Link href="/doctor/profile" className="h-12 w-12 rounded-xl bg-white/10 border border-white/20 flex items-center justify-center text-white font-bold text-lg hover:bg-white/20 transition backdrop-blur-sm">
        ...
        <div className="text-white relative z-10 mb-4 px-1">
          <h1 className="text-3xl font-bold">Hello, Dr. {doctor?.lastName || doctor?.firstName}</h1>
          <p className="text-blue-100 opacity-90 text-sm mt-1">
            {hospitalAppointments.length > 0 ? (
              <>You have <span className="font-bold text-white">{hospitalAppointments.length}</span> appointments today at {currentHospitalName}</>
            ) : (
              <>No appointments scheduled at {currentHospitalName}</>
            )}
          </p>
        </div>
      ...
      <div className="px-4 mt-20 space-y-6">
        ...
        <div>
          <div className="flex items-center justify-between mb-3 px-1">
            ...
            <Link href="/doctor/appointments" className="text-blue-600 text-sm font-semibold flex items-center">
              View All <ChevronRight className="w-4 h-4" />
            </Link>
          </div>
          <div className="space-y-3 pb-6">
            {hospitalAppointments
              .filter(apt => ['BOOKED', 'CHECKED_IN'].includes(apt.status))
              .sort((a, b) => (a.tokenNumber || 0) - (b.tokenNumber || 0))
              .slice(0, 5)
              .map(apt => (
                <div key={apt._id} className="bg-white p-3 rounded-xl shadow-sm border border-slate-100 flex items-center gap-3 hover:shadow-md transition-shadow">
                  <div className={`h-10 w-10 rounded-full flex items-center justify-center font-bold text-sm shadow-sm ${
                    apt.status === 'CHECKED_IN' ? 'bg-green-100 text-green-700' : 'bg-slate-100 text-slate-600'
                  }`}>
                    #{apt.tokenNumber || '?'}
                  </div>
